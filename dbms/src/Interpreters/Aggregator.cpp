    ...
    Arena * aggregates_pool) const
{
    ...
    const auto alloc_func = [this, aggregates_pool](const typename Method::FT & key) {
        // auto * aggregate_data = agg_states_batch_allocator->allocate();
        auto * aggregate_data = aggregates_pool->alignedAlloc(total_size_of_aggregate_states, align_aggregate_states);
...
template <bool collect_hit_rate, bool only_lookup, bool using_ph_map, typename Method>
    ...
    AggregateStatesBatchAllocator * agg_states_batch_allocator) const
{
    ...
    if (params.aggregates_size == 0)
    {
        ...
        {
            RUNTIME_CHECK_MSG(false, "ph map logic doesn't impl empty agg func");
        }
        else
        {
            /// For all rows.
    ...
    if constexpr (std::is_same_v<Method, AggregatedDataVariants::AggregationMethod_key8>)
    {
        ...
        if constexpr (using_ph_map)
        {
            RUNTIME_CHECK_MSG(false, "ph map logic doesn't impl key8 logic");
        }
        else
        {
            for (AggregateFunctionInstruction * inst = agg_process_info.aggregate_functions_instructions.data(); inst->that;
    ...
    for (size_t i = agg_process_info.start_row; i < agg_process_info.start_row + agg_size; ++i)
    {
        ...
        auto emplace_result_holder
            = emplaceOrFindKey<only_lookup>(method, state, i, *aggregates_pool, sort_key_containers, agg_states_batch_allocator);
    ...
    size_t thread_num)
{
    ...
    if (!result.inited())
    {
        ...
        // todo fix, using method check
        if (method_chosen == AggregatedDataVariants::Type::key64_phmap)
    ...
    Arena * arena) const
{
    if constexpr (Method::using_ph_map)
    {
        RUNTIME_CHECK_MSG(false, "convertToBlockImplFinal not impl for ph map");
    }
    else
    {
...
void NO_INLINE Aggregator::convertToBlocksImplFinal(
    ...
    Arena * arena) const
{
    if constexpr (std::decay_t<Method>::using_ph_map)
    {
        RUNTIME_CHECK_MSG(false, "convertToBlocksImplFinal not impl for ph map");
    }
    else
    {
    ...
    std::vector<AggregateColumnsData> & aggregate_columns_vec) const
{
    if constexpr (std::decay_t<Method>::using_ph_map)
    {
        RUNTIME_CHECK_MSG(false, "convertToBlocksImplNotFinal not impl for ph map");
    }
    else
    {
#ifndef NDEBUG
...
{
    // todo no member named 'emplaceKey' in 'DB::ColumnsHashing::HashMethodOneNumberPhMap<std::pair<const unsigned long, char *>, char *, unsigned long>'
    if constexpr (Method::using_ph_map)
    {
        RUNTIME_CHECK_MSG(false, "ph map should not got here");
    }
    else
    {
...
void NO_INLINE Aggregator::destroyImpl(Table & table) const
{
    if constexpr (Method::using_ph_map)
    {
        RUNTIME_CHECK_MSG(false, "destroyImpl not impl for ph map");
    }
    else
    {
...
Block MergingBuckets::getData(size_t concurrency_index)
{
    ...
    const auto & one_data = *data[0];
    if (one_data.type == AggregatedDataVariants::Type::key64_phmap)
    {
        return getDataForPhMap();
    }
...
Block MergingBuckets::getDataForPhMap()
{
    ...
    auto & phmap = reinterpret_cast<AggregatedDataVariants::AggregationMethod_key64_phmap *>(data[0]->aggregation_method_impl)->data;
    auto iter = phmap.begin();
    while (iter != phmap.end())
    {
        for (size_t i = 0; i < params.max_block_size; ++i)
        {
            if unlikely (iter == phmap.end())
                break;

            const auto key = reinterpret_cast<const UInt64>(iter->first);
            // todo Method::insertKeyIntoColumns
            key_column->insert(Field(static_cast<Int64>(key)));

            AggregateDataPtr agg_states = iter->second;
            for (size_t j = 0; j < params.aggregates_size; ++j)
            {
        ...
        block.getByPosition(0).column = std::move(key_column);
        for (size_t agg_func_idx = 0; agg_func_idx < params.aggregates_size; ++agg_func_idx)
        {
            block.getByName(params.aggregates[agg_func_idx].column_name).column = 
                std::move(agg_func_columns[agg_func_idx]);
        }
        single_level_blocks.push_back(block);
