        ...
        AggProcessInfo & agg_process_info) const
{
    ...
    LOG_DEBUG(log, "gjt debug executeImplPrefetch prefetch beg");
    ...
    LOG_DEBUG(log, "gjt debug executeImplPrefetch prefetch done");
    ...
    for (size_t i = 0; i < total_rows; ++i)
    {
        ...
        bool inserted = false;
        method.data.emplace(key, lookup_result, inserted);
        if (inserted)
        {
            aggregate_data = agg_states_batch_allocator.allocate();
            // aggregate_data = aggregates_pool->alignedAlloc(total_size_of_aggregate_states, align_aggregate_states);
            *reinterpret_cast<UInt64 *>(aggregate_data) = keyHolderGetKey(key);
    ...
    LOG_DEBUG(log, "gjt debug executeImplPrefetch emplace done, {}", agg_process_info.aggregate_functions_instructions.size());
    ...
    LOG_DEBUG(log, "gjt debug executeImplPrefetch addBatch done");
}
...
Block MergingBuckets::getDataByAggregateStatesBatchAllocator()
{
    ...
    LOG_DEBUG(log, "gjt debug begin getDataByAggregateStatesBatchAllocator, {}, {}", aggregator.getHeader(true).dumpStructure(), aggregator.params.aggregates_size);
    ...
    {
        // LOG_DEBUG(log, "gjt debug iter one_batch {}", iter_j++);
        // TODO block too small. consider max_block_size
        for (size_t i = 0; i < one_batch.second; ++i)
        {
        // LOG_DEBUG(log, "gjt debug iter inside one_batch {}, {}, {}", iter_j, i, params.aggregates_size);
            ...
            {
        LOG_DEBUG(log, "gjt debug iter new block, {}, {}", iter_j, i);
                Block res = header.cloneEmpty();
    ...
    LOG_DEBUG(log, "gjt debug iter one_batch done");
    if (!key_column->empty())
    {
        Block res = header.cloneEmpty();
        res.getByPosition(0).column = std::move(key_column);
    ...
    current_bucket_num++;
    LOG_DEBUG(log, "gjt debug begin getDataByAggregateStatesBatchAllocator done");
