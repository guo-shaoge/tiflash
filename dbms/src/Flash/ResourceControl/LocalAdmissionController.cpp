...
void analyzeRUUsage(std::vector<double>& cpu_ru_stats, const LoggerPtr & log, const KeyspaceID & keyspace_id, const std::string & name)
{
...
LocalAdmissionController::doRequestGAC(const TokenBucketRequestPBVec & last_round_failed_requests)
{
    ...
    while (!stopped.load())
    {
        ...
        if unlikely (need_handle_prev_req)
        {
