...
void AggregateContext::initConvergentPrefix()
{
    ...
    size_t total_alloc_continue_real_used_bytes = 0;
    size_t total_alloc_continue_wasted_bytes = 0;
    for (size_t i = 0; i < max_threads; ++i)
    {
        ...
        LOG_DEBUG(log, "gjt debug thread data [{}]: hashmap kv pair num: {}, hashmap total cells: {}, hashmap bytes size: {}, hashmap and arena byte size: {}, align alloc real used bytes: {}, align alloc wasted bytes: {}, alloc real used bytes: {}, alloc continue real used bytes: {}, alloc continue wasted bytes: {}",
                ...
                many_data[i]->allocContinueRealUsedBytes(), many_data[i]->allocContinueWastedBytes());
        ...
        total_alloc_continue_real_used_bytes += many_data[i]->allocContinueRealUsedBytes();
        total_alloc_continue_wasted_bytes += many_data[i]->allocContinueWastedBytes();

        total_src_rows += threads_data[i]->src_rows;
        total_src_bytes += threads_data[i]->src_bytes;
    }

    LOG_DEBUG(log, "gjt debug total block bytes: hashmap kv pair num: {}, hashmap total cells: {}, hashmap bytes size: {}, hashmap and arena byte size: {}, arena align alloc real used bytes: {}, arena align alloc wasted bytes: {}, alloc real used bytes: {}, alloc continue real used bytes: {}, alloc continue wasted bytes: {}",
            total_hash_map_rows, total_cells, total_hash_map_bytes, total_hash_map_and_arena_bytes, total_align_alloc_real_used_bytes, total_align_alloc_wasted_bytes, total_alloc_real_used_bytes, total_alloc_continue_real_used_bytes, total_alloc_continue_wasted_bytes);
